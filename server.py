...
class ScoutServer(object):
    ...
    def index(self, m='', e=''):
        ...
        for team in data:
            table += '''
                <tr role="row">
                    <td><a href="team?n={0}">{0}</a></td>'''.format(
                team['Team'])
            for i, key in enumerate(game.AVERAGE_FIELDS):
                if key != 'team':
                    table += '''
        ...
        with open('web/index.html', 'r') as file:
            page = file.read()
        return page.format(table, cherrypy.session['event'],
                           cherrypy.session['mode'])
    ...
    def team(self, n="238"):
        ...
        entries = cursor.execute(
            'SELECT * FROM scout WHERE Team=? ORDER BY Match DESC',
            (n, )).fetchall()
        ...
        if len(averages):
            ...
            s = [0] * len(game.AVERAGE_FIELDS
                          )  #generate zeros if no data exists for the team yet
        ...
        if (len(entries) < 4):
            ...
            teamEvents = globalcursor.execute(
                'SELECT * FROM teamEvents WHERE Team=?', (n, )).fetchone()
            if teamEvents:
                for i in range(1, 10):
                    if teamEvents['Event' + str(i)]:
                        if teamEvents['Event'
                                      + str(i)] != cherrypy.session['event']:
        ...
        if lastEvent:
            try:
                ...
                oldAverages = oldcursor.execute(
                    'SELECT * FROM averages WHERE team=?', (n, )).fetchall()
                assert len(oldAverages
        ...
        for comment in comments:
            commentstr += '<div class="commentbox"><p>{0}</p></div>'.format(
                comment[1])
        ...
        for e in entries:
            ...
            <tr role="row" {5}>
                ...
                e['Match'], *text.values(), 'style="color: #B20000"'
                if e['Flag'] else '', e['Match'], e['Flag'], e['Key'])
    ...
    def flag(self, num='', match='', flagval=0):
        ...
        conn.row_factory = sql.Row
        cursor = conn.cursor()
        cursor.execute('UPDATE scout SET Flag=? WHERE Team=? AND Match=?',
                       (int(not int(flagval)), num, match))
        conn.commit()
        conn.close()
    ...
    def recalculate(self):
        ...
        if not cherrypy.session['auth'] == serverinfo.AUTH:
            raise cherrypy.HTTPError(
                401,
                "Not authorized to perform recalculate. Please log in and try again."
            )
        data = conn.cursor().execute(
            'SELECT * FROM averages ORDER BY apr DESC').fetchall()
        for team in data:
    ...
    def teams(self, n1='', n2='', n3='', n4='', stat1='', stat2=''):
        ...
        for index, n in enumerate(nums):
            ...
            if not n.isdigit():
                raise cherrypy.HTTPError(
                    400, "You fool! Enter NUMBERS, not letters.")
            ...
            output += '''<div class="comparebox_container">
                    ...
                    <div class="statbox_container">
                        ...
                        <div id="stats">
                            <p class="statbox" style="font-weight:bold">Average match:</p>'''.format(
                entry['Team'], entry['APR'])
            for key in game.AVERAGE_FIELDS:
                if (key != 'team') and (key != 'apr'):
                    output += '''<p class="statbox">{0}: {1}</p>'''.format(
                        key, entry[key])
            output += '''       </div>
                            </div>
                         </div>'''
            if ((len(nums) == 2 and index == 0)
                    or (len(nums) != 2 and index == 1)):
                output += '</div><div>'
        output += '</div>'
        ...
        dataset = []
        colors = [
            "#FF0000", "#000FFF", "#1DD300", "#C100E3", "#AF0000", "#000666",
            "#0D5B000", "#610172"
        ]
        ...
        for idx, n in enumerate(nums):
            if not n:
                continue
            entries = cursor.execute(
                'SELECT * FROM scout WHERE Team=? ORDER BY Match ASC',
                (n, )).fetchall()
            ...
            teamCharts += '''// GRAPH
                            ...
                            '''.format(n + "_1", "Team " + n + stat1,
                                       "team" + n + "stat1", colors[idx])
            if stat2:
                teamCharts += '''// GRAPH
                graph{0} = new AmCharts.AmGraph();
                graph{0}.title = "{1}";
                ...
                graph{0}.balloonText = "{1}<br><b><span style='font-size:14px;'>[[value]]</span></b>";
                chart.addGraph(graph{0});
                '''.format(n + "_2", "Team " + n + stat2, "team" + n + "stat2",
                           colors[4 + idx])
        with open('web/teams.html', 'r') as file:
            page = file.read()
        return page.format(
            str(dataset).replace("'", '"'), teamCharts, stat1, stat2 + "2",
            output)
    ...
    def alliances(self,
        ...
        with open('web/alliances.html', 'r') as file:
            page = file.read()
        return page.format(
            round((1 - prob_red) * 100, 1), blue_score, *teamsBlue,
    ...
    def submit(self, auth='', data='', event='', team='', comment=''):
        ...
        if auth == serverinfo.AUTH:
            ...
            if m:
                match = next((item for item in m
                              if (item['match_number'] == d['Match']) and (
                                  item['comp_level'] == 'qm')))
            ...
            raise cherrypy.HTTPError(
                401, "Error: Not authorized to submit match data")
    ...
    def calcavg(self, n, event):
        ...
        globalconn.row_factory = sql.Row
        globalcursor = globalconn.cursor()
        teamEntry = globalcursor.execute(
            'SELECT * FROM teamEvents WHERE Team=?', (n, )).fetchone()
        if (teamEntry):
            for i in range(1, 10):
                if teamEntry['Event' + str(i)] == event:
                    ...
                    if not teamEntry['Event' + str(i)]:
                        updateString = 'UPDATE teamEvents SET Event' + str(
                            i) + " = '" + event + "' WHERE Team=" + str(n)
                        globalcursor.execute(updateString)
                        break
        else:
            globalcursor.execute(
                'INSERT INTO teamEvents (Team,Event1) VALUES (?,?)',
                (n, event))
        globalconn.commit()
        globalconn.close()
        ...
        entries = cursor.execute(
            'SELECT * FROM scout WHERE Team=? AND flag=0 ORDER BY Match DESC',
            (n, )).fetchall()
    ...
    def edit(self, key='', **params):
        ...
        cursor = conn.cursor()
        entries = cursor.execute(
            'SELECT * from scout ORDER BY flag DESC, Team ASC, Match ASC'
        ).fetchall()
        ...
        for e in entries:
            combobox += '''<option id="{0}" value="{0}">{1} Team {2}: Match {3}</option>\n'''.format(
                e['Key'], "*" if e['Flag'] else "", e['Team'], e['Match'])

        #Grab the currently selected entry
        entry = cursor.execute('SELECT * from scout WHERE key=?',
                               (key, )).fetchone()
        conn.close()
        ...
        for key in game.SCOUT_FIELDS:
            ...
            if (i < len(game.SCOUT_FIELDS) / 2):
                leftEdit += '''<div><label for="team" class="editLabel">{0}</label>
                            <input class="editNum" type="number" name="{0}" value="{1}"></div>'''.format(
                    key, entry[key])
            else:
        ...
        with open('web/edit.html', 'r') as file:
            page = file.read()
        return page.format(combobox, entry['Team'], entry['Match'],
                           entry['Key'], leftEdit, rightEdit)
